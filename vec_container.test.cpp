...
{
	geosimd::vec_container<my_vector, std::array<my_vector, 3>> a;
	a[0] = my_vector{1, 2, 3, 4};
	a[1] = my_vector{5, 6, 7, 8};
	a[2] = my_vector{9, 10, 11, 12};

	geosimd::vec_container<my_vector, std::array<my_vector, 3>> b;
	...
	geosimd::vec_container<my_vector, std::array<my_vector, 3>> expected_sum;
...
TESTCASE(geosimd_vec_container_add_other_container)
{
	...
	geosimd::vec_container<my_vector, std::list<my_vector>> b;
...
TESTCASE(geosimd_vec_container_mul_div)
{
	...
	geosimd::vec_container<my_vector, std::array<my_vector, 3>> expected_prod;
